...
impl IcebergInspector {
    ...
    fn extract_file_info(
        ...
        let num_snapshots = snapshots.map(|arr| arr.len()).unwrap_or(0);
    ...
    fn extract_sort_order(&self, metadata: &serde_json::Value) -> String {
        ...
        let sort_order = match sort_orders.iter().find(|so| {
            so.get("order-id").and_then(|id| id.as_i64()) == Some(default_sort_order_id)
        }) {
            Some(order) => order,
            None => return "(unsorted)".to_string(),
        };
        ...
        let sort_cols: Vec<String> = fields
            ...
            .filter_map(|f| {
                ...
                let null_order = f.get("null-order").and_then(|n| n.as_str()).unwrap_or("nulls-first");
                    ...
                    if let Some(summary) = snapshot.get("summary").and_then(|s| s.as_object()) {
                        ...
                        if options.verbosity >= VerbosityLevel::Verbose {
                            ...
                            if let Some(manifest_list) = snapshot.get("manifest-list").and_then(|m| m.as_str()) {
                                ...
                                details.insert("Manifest List".to_string(),
                                    manifest_list.split('/').last().unwrap_or("unknown").to_string());
                            }
...
impl PhysicalInspector for IcebergInspector {
    ...
    async fn extract_metadata(
        ...
        let file_info = self.extract_file_info(&metadata, &metadata_path, options);
