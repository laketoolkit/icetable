...
pub fn apply_filter(batch: RecordBatch, filter_expr: &str) -> Result<RecordBatch> {
    ...
    compute::filter_record_batch(&batch, &filter_array)
        .map_err(|e| Error::General(format!("Failed to apply filter: {}", e)))
}
...
fn evaluate_filter_expression(batch: &RecordBatch, expr: &str) -> Result<BooleanArray> {
    ...
    if expr.contains(" AND ") {
        ...
        return compute::and(&left, &right)
            .map_err(|e| Error::General(format!("Failed to evaluate AND: {}", e)));
    }
    ...
    if expr.contains(" OR ") {
        ...
        return compute::or(&left, &right)
            .map_err(|e| Error::General(format!("Failed to evaluate OR: {}", e)));
    }
    ...
    for op in &operators {
        if let Some(pos) = expr.find(op) {
            ...
            let column_index = schema
                ...
                .map_err(|_| Error::General(format!("Column '{}' not found", column_name)))?;
    ...
    match column.data_type() {
        DataType::Int64 => {
            let array = column
                ...
                .ok_or_else(|| Error::General("Failed to downcast Int64Array".to_string()))?;
            let value = value_str
                .parse::<i64>()
                .map_err(|_| Error::General(format!("Cannot parse '{}' as Int64", value_str)))?;
            ...
            Ok(match op {
                ...
                _ => return Err(Error::General(format!("Unsupported operator: {}", op))),
            })
        }
        DataType::Float64 => {
            let array = column
                ...
                .ok_or_else(|| Error::General("Failed to downcast Float64Array".to_string()))?;
            let value = value_str
                .parse::<f64>()
                .map_err(|_| Error::General(format!("Cannot parse '{}' as Float64", value_str)))?;
        ...
        DataType::Utf8 => {
            let array = column
                ...
                .ok_or_else(|| Error::General("Failed to downcast StringArray".to_string()))?;
        ...
        _ => Err(Error::General(format!(
            "Filtering not supported for data type: {:?}",
            column.data_type()
        ))),
    }
}
