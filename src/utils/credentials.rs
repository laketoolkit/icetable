...
impl CredentialSource {
    ...
    pub fn describe(&self) -> String {
        match self {
            ...
            CredentialSource::EnvVar(var) => format!("environment variable '{}'", var),
            CredentialSource::File(path) => format!("file '{}'", path.display()),
            CredentialSource::IamRole => "IAM role (auto-detected)".to_string(),
            CredentialSource::OAuth2 => "OAuth2 (auto-refreshed)".to_string(),
        }
    }
...
mod tests {
    ...
    fn test_env_var_credential() {
        ...
        let cred = CredentialSource::EnvVar("TEST_TOKEN".to_string());
        assert_eq!(cred.resolve().unwrap(), Some("secret123".to_string()));
        assert_eq!(cred.describe(), "environment variable 'TEST_TOKEN'");
        unsafe {
    ...
    fn test_iam_role_credential() {
        ...
        assert_eq!(cred.describe(), "IAM role (auto-detected)");
    }
