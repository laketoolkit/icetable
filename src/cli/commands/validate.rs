...
use crate::cli::parser::{CatalogContext, ValidateArgs};
...
impl ValidateCommand {
    ...
    pub async fn execute(args: ValidateArgs, ctx: &CatalogContext) -> Result<()> {
        use super::constants::MEMORY_MEDIUM_OPS;
        with_resource_limits(MEMORY_MEDIUM_OPS, Self::execute_inner(args, ctx)).await
    }

    async fn execute_inner(args: ValidateArgs, ctx: &CatalogContext) -> Result<()> {
