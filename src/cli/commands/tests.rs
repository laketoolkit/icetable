...
mod common_tests {
    ...
    fn test_extract_table_name_with_trailing_slash() {
        assert_eq!(
            extract_table_name("s3://bucket/database/events/"),
            "events"
        );
    }
    ...
    fn test_extract_table_name_local_path() {
        assert_eq!(
            extract_table_name("/home/user/data/my_table"),
            "my_table"
        );
    }
...
mod constants_tests {
    ...
    fn test_memory_constants_ordering() {
        ...
        assert!(MEMORY_CONFIG_OPS < MEMORY_INIT_OPS);
        assert!(MEMORY_INIT_OPS < MEMORY_LIGHT_OPS);
        assert!(MEMORY_LIGHT_OPS < MEMORY_MEDIUM_OPS);
        assert!(MEMORY_MEDIUM_OPS < MEMORY_HEAVY_OPS);
        assert!(MEMORY_HEAVY_OPS < MEMORY_INTENSIVE_OPS);
    }
...
mod cli_parse_tests {
    ...
    fn test_cli_catalog_config_with_uri() {
        let cli = Cli::try_parse_from([
            "icetable",
            "--catalog-uri", "https://polaris.example.com",
            "--catalog-warehouse", "my_warehouse",
            "ls",
        ]).unwrap();
...
mod admin_args_tests {
    ...
    fn test_admin_config_ls() {
        let cli = Cli::try_parse_from([
            ...
            "admin",
            "config",
            "ls", // Config requires a subcommand
        ]).unwrap();
